...
注意：本项目目前还没有完全写完，只是测试阶段可以运行。纯 P2P UDP 打洞不保证所有网络都成功。对称 NAT、校园网、公司网、运营商 CGNAT 或严格防火墙环境可能无法连接，后续可加入服务器中继模式作为兜底。
...
- 允许个人非商业学习、测试、运行。
- 禁止未经授权的商业使用。
- 分发、二次修改、再发布、衍生项目、公开部署或组织内部使用均需要作者授权。
- 普通个人非商业测试一般不会追究责任。
- 项目完成前不保证稳定性、可用性或兼容性。
- 项目全部写完后，作者可能会更换为更开放的协议。
