...
fn parse_stun_response(resp: &[u8], expected_txn_id: &[u8; 12]) -> Option<SocketAddr> {
    ...
    if resp[0] != 0x01 || resp[1] != 0x01 { return None; } // Not a Binding Response

    // 校验 magic cookie
    if resp[4..8] != [0x21, 0x12, 0xA4, 0x42] { return None; }

    // 校验 transaction ID
    if resp[8..20] != expected_txn_id[..] { return None; }
...
pub async fn get_public_address() -> Result<(std::net::UdpSocket, SocketAddr), String> {
    tokio::task::spawn_blocking(|| {
        ...
        // 国内在前，海外在后
        let stun_servers: Vec<&str> = cn_servers.iter().chain(intl_servers.iter()).copied().collect();
