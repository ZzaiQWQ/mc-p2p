...
#[tauri::command]
async fn detect_mc_port(app: AppHandle) -> Result<u16, String> {
...
async fn host_step2_connect(
    ...
    let socket = {
        state.pending_sockets.lock().unwrap()
            .remove(&stun_addr)
...
async fn guest_step2_connect(
    ...
    let connection = endpoint.connect(actual_host_addr, "minecraft.p2p")
        .map_err(|e| e.to_string())?
        .await
        .map_err(|e| e.to_string())?;
    ...
    emit_log(&app, "======== 联机准备完毕 ========");
    emit_log(&app, format!(">> 游戏内局域网列表将自动刷出房间（端口 {}）！ <<", actual_port));
    Ok(())
}

/// 取消所有代理任务（用于重置/退出时）
#[tauri::command]
fn reset_connections(state: tauri::State<'_, AppState>) {
    cancel_all_proxies(&state);
